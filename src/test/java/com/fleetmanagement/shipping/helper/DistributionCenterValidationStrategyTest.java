...
class DistributionCenterValidationStrategyTest {
	...
	@InjectMocks
	DistributionCenterValidationStrategy strategy;
	...
	public void setUp() {
		...
		dBag2.setDeliveryPoint(deliveryPointDto2);
		dBag2.setCreatedAt(LocalDateTime.now());
		
		dPackageForDelivery2 = new PackageDto();
		dPackageForDelivery2.setBarcode("P1111111118");
		...
		dPackageForDelivery3.setBarcode("P1111111119");
		dPackageForDelivery3.setDeliveryPoint(deliveryPointDto2);
		dPackageForDelivery3.setBag(new BagDto());
		dPackageForDelivery3.setState(4);
		
		deliveryList = new ArrayList<>(Arrays.asList(deliveryWithBagBarcode1, deliveryWithBagBarcode2, delivery2, delivery3));
	}
	...
	public void testUnload() {
		...
		when(packageService.getPackagesByBagId(any())).thenReturn(Arrays.asList(dPackageForDelivery3));
		...
		assertNotNull(strategy.unload(deliveryList, 2L));
	}
